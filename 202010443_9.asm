...
# 202010443_9.asm -- 9x9 sudoku solver in MIPS

.macro			get_start(%result, %index)			
			li	$t9, 3					#$t9 = 3
			div	%index, $t9				#index // 3
			mflo	%result					#result = index // 3
			mult	%result, $t9				#result * 3
			mflo	%result					#result = result * 3
.end_macro
...
input_forloop:		beq	$t1, $s1, input_endloop   		#for colIndex in range(size)
			...
			get_address($t2, %rowIndex, $t1, $a2, $a1, $a3)	#$t2 = address (grid[row][col])
									#getting the appropriate nibble from user input
			add	$t4, $a0, $t1				#$t6 = input baseAddress + offset
			lb	$t3, 0($t4)				#$t5 = nibble
			sw	$t3, 0($t2)				#store nibble at address
...
next_col_forloop:	beq	$t1, $s6, next_col_endloop 		#for colIndex in range(size)
			...
			lw	$t3, 0($t2)				#if content at the addess == 0 (30 in asciii), then return address
...
checkcol_endloop:
			...
			addi	$s1, $t1, 3				#$s1 = boundary for row = row+3
squarerow_forloop:	beq	$t1, $s1, squarerow_endloop		#for row in range(rowStart, rowStart+3)
			
			get_start($t2, $s6)				#$t2 = colStart = starting colIndex of grid
			addi	$s2, $t2, 3				#$s2 = boundary for col = col+3
squarecol_forloop:	beq	$t2, $s2, squarecol_endloop		#for col in range(colStart, colStart+3)
...
inp:		.asciiz "testing"
		.space 	4
grid:		.word 	1, 1, 1, 1, 1, 1, 1, 1, 1
		.word 	2, 2, 2, 2, 2, 2, 2, 2, 2
		.word 	3, 3, 3, 3, 3, 3, 3, 3, 3
		.word 	4, 4, 4, 4, 4, 4, 4, 4, 4
